...
func TestLinesWidget_BothZeroReturnsEmpty(t *testing.T) {
	...
	if got := Lines(ctx, cfg); got != "" {
		t.Errorf("Lines both zero: expected empty string, got %q", got)
	}
}
...
func TestLinesWidget_OnlyAdditions(t *testing.T) {
	...
	if !strings.Contains(got, "+42") {
		t.Errorf("Lines only additions: expected '+42' in output, got %q", got)
	}
	if strings.Contains(got, "-") {
		t.Errorf("Lines only additions: unexpected '-' in output, got %q", got)
	}
}
...
func TestLinesWidget_OnlyRemovals(t *testing.T) {
	...
	if !strings.Contains(got, "-17") {
		t.Errorf("Lines only removals: expected '-17' in output, got %q", got)
	}
	if strings.Contains(got, "+") {
		t.Errorf("Lines only removals: unexpected '+' in output, got %q", got)
	}
}
...
func TestLinesWidget_BothPresent(t *testing.T) {
	...
	if !strings.Contains(got, "+100") {
		t.Errorf("Lines both: expected '+100' in output, got %q", got)
	}
	if !strings.Contains(got, "-23") {
		t.Errorf("Lines both: expected '-23' in output, got %q", got)
	}
}
...
func TestLinesWidget_NilCostData(t *testing.T) {
	...
	if got := Lines(ctx, cfg); got != "" {
		t.Errorf("Lines nil cost data: expected empty string, got %q", got)
	}
}
