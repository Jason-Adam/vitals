...
vitals is a Go binary that renders a terminal statusline for Claude Code sessions. Claude Code pipes JSON to stdin on every tick; this binary parses it, gathers supplementary data (transcript state, git status, environment counts), and prints a styled multi-line statusline to stdout. The entire cycle must complete in single-digit milliseconds because it runs on every keypress/tick.
...
**Never write to stderr**: Claude Code owns the terminal. Any stderr output corrupts the display. Debug logging goes to `~/.claude/plugins/vitals/debug.log` and is gated behind `TAIL_CLAUDE_HUD_DEBUG=1`.
